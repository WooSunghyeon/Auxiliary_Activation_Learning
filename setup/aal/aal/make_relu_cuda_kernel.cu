/*
 * Packbits and upackbits for 1bits for ReLU.
 */
...
__global__ void make_relu_forward_kernel(const scalar_t* __restrict__ data,
...
std::tuple<Tensor, Tensor, IntArrayRef> make_relu_forward_cuda(Tensor data) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(data.scalar_type(), "make_relu_forward", ([&] {
    make_relu_forward_kernel<scalar_t><<<blocks, threads>>>(
...
template <typename scalar_t>
__global__ void make_relu_backward_kernel(scalar_t* __restrict__ grad_output,
                                                   int32_t* __restrict__ mask,
                                                   scalar_t* __restrict__ output,
                                                   int N) {
...
Tensor make_relu_backward_cuda(Tensor grad_output, Tensor mask, IntArrayRef data_size) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad_output.scalar_type(), "make_relu_backward", ([&] {
      make_relu_backward_kernel<scalar_t><<<blocks, threads>>>(
        grad_output.data_ptr<scalar_t>(), mask.data_ptr<int32_t>(), unpack.data_ptr<scalar_t>(),
        n_elements);
  }));
